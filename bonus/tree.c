...
void my_tree(void)
{
    ...
    project->my_window = malloc(sizeof(window_t));
    ...
    project->my_window->window = sfRenderWindow_create(mode, "MY_SCREENSAVER", sfFullscreen, NULL);
    sfRenderWindow_setFramerateLimit(project->my_window->window, 60);
    while (sfRenderWindow_isOpen(project->my_window->window)) {
        ...
        sfRenderWindow_drawVertexArray(project->my_window->window, array, NULL);
        sfRenderWindow_display(project->my_window->window);
...
void branch(float change, sfVertex my_point1, sfVertex my_point2, project_t *var, float angle, int a)
{
    ...
    if (sfRenderWindow_isOpen(var->my_window->window)) {
        sfVertex vertex1;
        sfVertex vertex2;
        sfVertexArray *array = sfVertexArray_create();
        sfVertexArray_setPrimitiveType(array, sfLines);
        ...
        if (result > 4) {
            ...
            sfVertexArray_append(array, vertex1);
            sfVertexArray_append(array, vertex2);
            sfRenderWindow_drawVertexArray(var->my_window->window, array, NULL);
            sfRenderWindow_display(var->my_window->window);
